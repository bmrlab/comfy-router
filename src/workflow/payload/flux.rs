use super::{ComfyUIPrompt, ControlNetPayload, Image, LoRAPayload, Model};
use crate::workflow::{fetch::FetchHelper, payload::CurrentNodeId};
...
impl FluxWorkflowPayload {
    ...
    pub async fn into_comfy_prompt(
        ...
        // UNet
        let load_unet_node_id = current_node_id.get();
        let unet_name = fetch_helper.add(&self.unet, "models/unet").await;
        prompt.insert(
            load_unet_node_id.clone(),
            json!({
                "inputs": {"unet_name": unet_name, "weight_dtype": self.unet_dtype},
                "class_type": "UNETLoader",
            }),
        );
        unet_node = (load_unet_node_id.clone(), 0);

        // vae
        let load_vae_node_id = current_node_id.get();
        let vae_name = fetch_helper.add(&self.vae, "models/vae").await;
        prompt.insert(
            load_vae_node_id.clone(),
            json!({
                "inputs": {"vae_name": vae_name},
                "class_type": "VAELoader",
            }),
        );
        let vae_node = (load_vae_node_id.clone(), 0);

        // CLIP
        let load_clip_node_id = current_node_id.get();
        let t5xxl_name = fetch_helper.add(&self.t5xxl, "models/clip").await;
        let clip_name = fetch_helper.add(&self.clip, "models/clip").await;
        prompt.insert(
            load_clip_node_id.clone(),
            json!({
                "inputs": {
        ...
        clip_node = (load_clip_node_id.clone(), 0);
        ...
        // add guidance for Flux
        // only dev need this
        if self.unet_type == FluxUNet::Dev {
            let guidance_node_id = current_node_id.get();
            prompt.insert(
                guidance_node_id.clone(),
                json!({
                    "inputs": {
                        "conditioning": [positive_node.0.clone(), positive_node.1],
                        "guidance": self.cfg_scale
                    },
                    "class_type": "FluxGuidance"
                }),
            );
            positive_node = (guidance_node_id.clone(), 0);
        }
